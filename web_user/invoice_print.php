...
<main class='invoice'>
  ...
  <div class='invoice-wrapper'>
    ...
    <h2 class="invoice-info-value" style="font-size: 10px;">THÔNG TIN GIAO HÀNG</h2>
      <?php
        echo '
            <h2 class="invoice-billing-company">'.$kq_order[0]['fname'].' '.$kq_order[0]['lname'].'</h2>
            <p class="invoice-billing-address">
            
                <span>Điện thoại: '.$kq_order[0]['phone'].'</span>
                <span>Email: '.$kq_order[0]['email'].'</span>
                <span>Địa chỉ: '.$kq_order[0]['address'].'</span>
                <br>
                <span>Ghi chú: '.$kq_order[0]['notes'].'</span>
            </p>
        ';
        ...
        <div class="invoice-details">
            <div class="invoice-info">
                <span class="invoice-info-key">Số hóa đơn:</span>
                <span class="invoice-info-value">'.$kq_order[0]['invoice_id'].'</span>
            </div>
            <div class="invoice-info">
                <span class="invoice-info-key">Mã khách hàng:</span>
                <span class="invoice-info-value">'.$kq_order[0]['id_user'].'</span>
            </div>
            <div class="invoice-info">
                <span class="invoice-info-key">Ngày hết hạn:</span>
                <span class="invoice-info-value">'.$kq_order[0]['due_date'].'</span>
            </div>
            <div class="invoice-info">
                <span class="invoice-info-key">Trạng thái:</span>
    ...
    <?php
      ...
      <div>
        <div style="float: right; width: 17%;">
          <?php
            if (isset($tax)) {
              echo '<h3 style="color: purple;" >Thuế:  ' .number_format($tax). '</h3>';
              } else {
              echo '<td>0</td>';
            } 
            echo '<br><br>';
